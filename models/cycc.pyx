...
def normalized_cc(numpy.ndarray[DTYPE_t, ndim=2] s1, numpy.ndarray[DTYPE_t, ndim=2] s2,
    ...
    s11 = s1.copy()
    s22 = s2.copy()
    s11[numpy.isnan(s11)] = 0.
    s22[numpy.isnan(s22)] = 0.

    if norm1 < 0.:
        norm1 = numpy.linalg.norm(s11)
    if norm2 < 0.:
        norm2 = numpy.linalg.norm(s22)

    denom = norm1 * norm2
    if denom < 1e-9:  # To avoid NaNs
        denom = numpy.inf

    cc = numpy.real(numpy.fft.ifft(numpy.fft.fft(s11, fft_sz, axis=0) *
                                   numpy.conj(numpy.fft.fft(s22, fft_sz, axis=0)), axis=0))
...
def cdist_normalized_cc(numpy.ndarray[DTYPE_t, ndim=3] dataset1, numpy.ndarray[DTYPE_t, ndim=3] dataset2,
    ...
    if (norms1 < 0.).any():
        norms1 = numpy.array([numpy.linalg.norm(x[~numpy.isnan(x)]) for x in dataset1])
    if (norms2 < 0.).any():
        norms2 = numpy.array([numpy.linalg.norm(x[~numpy.isnan(x)]) for x in dataset2])
...
def y_shifted_sbd_vec(numpy.ndarray[DTYPE_t, ndim=2] ref_ts, numpy.ndarray[DTYPE_t, ndim=3] dataset, float norm_ref,
    ...
    if norm_ref < 0:
        norm_ref = numpy.array([numpy.linalg.norm(x[~numpy.isnan(x)]) for x in ref_ts])
    if (norms_dataset < 0.).any():
        norms_dataset = numpy.array([numpy.linalg.norm(x[~numpy.isnan(x)]) for x in dataset])
